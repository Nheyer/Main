def dict_maker(path,Depth_format_flag,reference_format_flag):
    ...
    gfloat = float(len(genotype))
    n_norm = gfloat / (4.100 * n_type)
    d_norm = gfloat / (5.175 * d_type)
        ...
        for i in range(len(var.samples)):
            if True: # Potentiol to blacklist things here
                ...
                try:
                    ...
                    if str(var.samples[i][Depth_format_flag]) == 'None':
                        vr = 0.0
                    else:
                        try:
                            vr = 1 - rr
                    ...
                    print(var.samples[i][Depth_format_flag])
                ...
                if genotype[i] == 'N':
                    ref_N = ref_N + n_norm * rr
                    var_N = var_N * n_norm * vr
                elif genotype[i] == 'A':
                    ref_A = ref_A + a_norm * rr
                    var_A = var_A + a_norm * vr
                elif genotype[i] == 'B':
                    ref_B = ref_B + b_norm * rr
                    var_B = var_B + b_norm * vr
                elif genotype[i] == 'C':
                    ref_C = ref_C + c_norm * rr
                    var_C = var_C + c_norm * vr
                elif genotype[i] == 'D':
                    ref_D = ref_D + d_norm * rr
                    var_D = var_D + d_norm * vr
...
def import_info():
    ...
    return [ref_input_path,ful_flag,ref_flag,unk_input_path]
